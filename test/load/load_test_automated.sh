...
EOF
    ...
    # Collect metrics
    local metrics=""
    if kubectl port-forward -n "$NAMESPACE" service/zen-cleaner-metrics 8080:8080 >/dev/null 2>&1 & then
        local pf_pid=$!
        sleep 2
...
collect_baseline_metrics() {
    ...
    local pod_name=$(kubectl get pods -n "$NAMESPACE" -l app=zen-cleaner -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")
    if [ -n "$pod_name" ]; then
