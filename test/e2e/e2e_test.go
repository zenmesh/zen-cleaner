...
// TestE2E_CleanerController requires a running Kubernetes cluster
// Run with: go test -tags=e2e ./test/e2e
// Or use: make test-e2e
func TestE2E_CleanerController(t *testing.T) {
	if testing.Short() {
...
func TestE2E_ResourceDeletion(t *testing.T) {
	...
	// Create cleanup policy with short TTL and dry-run
	policyGVR := schema.GroupVersionResource{
		Group:    "cleaner.zen-mesh.io",
		Version:  "v1alpha1",
		Resource: "zencleanerpolicies",
