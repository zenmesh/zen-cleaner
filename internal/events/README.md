...
This package provides a generic wrapper around Kubernetes event recording that can be used across zen-cleaner, zen-lock, zen-watcher, and other Kubernetes controller components. It enables consistent event recording patterns.
...
```go
import (
    "github.com/zenmesh/zen-cleaner/internal/pkg/events"
    "k8s.io/client-go/kubernetes"
)

// Create event recorder
recorder := events.NewRecorder(kubeClient, "zen-cleaner")
...
func NewEventRecorder(client kubernetes.Interface) *EventRecorder {
    return &EventRecorder{
        Recorder: events.NewRecorder(client, "zen-cleaner"),
    }
}

func (er *EventRecorder) RecordPolicyEvaluated(policy *v1alpha1.ZenCleanerPolicy, matched, deleted, pending int64) {
    er.Eventf(
        policy,
        corev1.EventTypeNormal,
        "PolicyEvaluated",
...
**After:**
```go
import "github.com/zenmesh/zen-cleaner/internal/pkg/events"
