...
import (
	...
	"github.com/zenmesh/zen-cleaner/internal/backoff"
...
// This interface allows PolicyReconciler to use shared rate limiter logic.
...
func getOrCreateRateLimiterShared(mgr RateLimiterManager, policy *v1alpha1.ZenCleanerPolicy) *ratelimiter.RateLimiter {
	...
	// Create new rate limiter using zen-cleaner/internal
...
type BatchDeleter interface {
	DeleteResourceWithBackoff(ctx context.Context, resource *unstructured.Unstructured, policy *v1alpha1.ZenCleanerPolicy, rateLimiter *ratelimiter.RateLimiter) error
...
func deleteBatchShared(
	...
	for i, resource := range batch {
		...
		if err := deleter.DeleteResourceWithBackoff(ctx, resource, policy, rateLimiter); err != nil {
			cleanerErr := cleanererrors.WithResource(
				cleanererrors.WithPolicy(err, policy.Namespace, policy.Name),
				resource.GetNamespace(),
				resource.GetName(),
			)
			cleanerErr.Type = "deletion_failed"
			recordError(policy.Namespace, policy.Name, "deletion_failed")
			errs = append(errs, cleanerErr)
			continue
		}
...
// This now delegates to zen-cleaner/internal/pkg/gc/ttl for the actual evaluation.
func calculateExpirationTimeShared(resource *unstructured.Unstructured, ttlSpec *v1alpha1.TTLSpec) (time.Time, error) {
	// Convert v1alpha1.TTLSpec to zen-cleaner/internal ttl.Spec
	sdkSpec := convertToSDKTTLSpec(ttlSpec)
	return sdkttl.CalculateExpirationTime(resource, sdkSpec)
}

// convertToSDKTTLSpec converts zen-cleaner's TTLSpec to zen-cleaner/internal's ttl.Spec.
...
type ResourceDeleterWithContext interface {
	DeleteResourceWithContext(ctx context.Context, resource *unstructured.Unstructured, policy *v1alpha1.ZenCleanerPolicy, rateLimiter *ratelimiter.RateLimiter) error
}
...
type ResourceDeleterWithoutContext interface {
	DeleteResourceWithoutContext(resource *unstructured.Unstructured, policy *v1alpha1.ZenCleanerPolicy, rateLimiter *ratelimiter.RateLimiter) error
}
...
func deleteResourceWithBackoffShared(
	...
	// Use zen-cleaner/internal backoff
...
func matchesSelectorsShared(resource *unstructured.Unstructured, target *v1alpha1.TargetResourceSpec) bool {
	...
	if target.LabelSelector != nil {
		...
		if err != nil {
			cleanerErr := cleanererrors.Wrap(err, "invalid_label_selector", "invalid label selector")
			logger := sdklog.NewLogger("zen-cleaner")
			logger.Error(cleanerErr, "Invalid label selector", sdklog.Operation("matches_selectors"), sdklog.ErrorCode("INVALID_LABEL_SELECTOR"))
			return false
		}
