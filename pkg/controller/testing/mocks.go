...
func (p *MockRateLimiterProvider) GetOrCreateRateLimiter(policy *v1alpha1.ZenCleanerPolicy) *ratelimiter.RateLimiter {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (d *MockBatchDeleterCore) DeleteBatch(ctx context.Context, batch []*unstructured.Unstructured, policy *v1alpha1.ZenCleanerPolicy, rateLimiter *ratelimiter.RateLimiter, reasons map[string]string) (int64, []error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	var deleted int64
	var errors []error
...
func (m *MockStatusUpdater) UpdateStatus(ctx context.Context, policy *v1alpha1.ZenCleanerPolicy, matchedCount, deletedCount, pendingCount int64) error {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.err
}
