...
This directory contains example `ZenCleanerPolicy` resources demonstrating various use cases and configurations.
...
2. Check controller logs:
   ```bash
   kubectl logs -n zen-cleaner-system -l app=zen-cleaner | grep "DRY RUN"
   ```
...
3. Verify policy is Active:
   ```bash
   kubectl get zencleanerpolicies <policy-name> -o jsonpath='{.status.phase}'
   ```
