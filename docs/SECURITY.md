...
This document provides security guidance for deploying and operating **zen-cleaner**. For **coordinated disclosure** and supported versions, see the root **[SECURITY.md](../SECURITY.md)** (contact: **security@zen-mesh.io**). **Zen Mesh Inc.** ([zen-mesh.io](https://zen-mesh.io)) sponsors OSS maintenance; other Zen Mesh offerings are governed separately.
...
zen-cleaner follows Kubernetes Pod Security Standards to ensure secure deployments.
...
zen-cleaner is compliant with **Restricted** Pod Security Standard:
...
Network policies restrict network access to the Zen Cleaner controller, following the principle of least privilege.
...
Enable audit logging to track all cleanup operations for security and compliance.

### Kubernetes Audit Policy

Create an audit policy to log Zen Cleaner controller operations:

```yaml
apiVersion: audit.k8s.io/v1
kind: Policy
rules:
# Log all delete operations by Zen Cleaner controller
- level: RequestResponse
  users: ["system:serviceaccount:zen-cleaner-system:zen-cleaner"]
  verbs: ["delete"]
  resources:
  - group: "*"
    resources: ["*"]
...
The Zen Cleaner controller logs all deletion operations:
...
The Zen Cleaner controller requires broad permissions to delete resources. Follow these best practices:
...
Deploy separate Zen Cleaner controllers per tenant namespace:
...
Be aware that cleanup operations count against resource quotas:
...
Use this checklist when deploying zen-cleaner:
