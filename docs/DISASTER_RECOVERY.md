...
This document describes disaster recovery procedures for zen-cleaner, including recovery from accidental mass deletions, backup strategies, emergency stop procedures, and rollback scenarios.
...
kubectl scale deployment zen-cleaner -n zen-cleaner-system --replicas=0

# Or delete the deployment entirely
kubectl delete deployment zen-cleaner -n zen-cleaner-system
```
...
kubectl annotate zencleanerpolicy <policy-name> -n <namespace> cleaner.zen-mesh.io/paused=true

# Or delete policies
kubectl delete zencleanerpolicy <policy-name> -n <namespace>
```
...
kubectl logs -n zen-cleaner-system deployment/zen-cleaner | grep "Deleting resource"

# Check controller metrics
kubectl port-forward -n zen-cleaner-system service/zen-cleaner-metrics 8080:8080
curl http://localhost:8080/metrics | grep zen_cleaner_resources_deleted_total
```
...
```bash
# Backup all cleanup policies
kubectl get zencleanerpolicies --all-namespaces -o yaml > gc-policies-backup-$(date +%Y%m%d).yaml

# Backup specific namespace policies
kubectl get zencleanerpolicies -n <namespace> -o yaml > gc-policies-<namespace>-backup.yaml
```
      ...
      template:
        spec:
          ...
          - name: backup
            ...
            - -c
            - |
              kubectl get zencleanerpolicies --all-namespaces -o yaml > /backup/gc-policies-$(date +%Y%m%d).yaml
...
```bash
# Fastest method - stops all cleanup operations immediately
kubectl scale deployment zen-cleaner -n zen-cleaner-system --replicas=0
```
...
kubectl get zencleanerpolicies --all-namespaces -o json | \
  jq -r '.items[] | "\(.metadata.namespace) \(.metadata.name)"' | \
  while read ns name; do
    kubectl delete zencleanerpolicy $name -n $ns
  done
```
...
echo "🚨 EMERGENCY STOP - Stopping Zen Cleaner Controller"
echo "This will stop all cleanup operations"

# Scale down controller
kubectl scale deployment zen-cleaner -n zen-cleaner-system --replicas=0

# Wait for pods to terminate
kubectl wait --for=delete pod -l app=zen-cleaner -n zen-cleaner-system --timeout=60s

echo "✅ Zen Cleaner Controller stopped"
...
```bash
# Restart controller
kubectl scale deployment zen-cleaner -n zen-cleaner-system --replicas=2
...
```bash
# List releases
helm list -n zen-cleaner-system

# Rollback to previous version
helm rollback zen-cleaner -n zen-cleaner-system

# Rollback to specific revision
helm rollback zen-cleaner <revision-number> -n zen-cleaner-system
```
...
kubectl rollout history deployment/zen-cleaner -n zen-cleaner-system

# Rollback to previous revision
kubectl rollout undo deployment/zen-cleaner -n zen-cleaner-system

# Rollback to specific revision
kubectl rollout undo deployment/zen-cleaner -n zen-cleaner-system --to-revision=<revision-number>
```
...
kubectl apply -f cleanup-policy-<name>-backup.yaml
```
...
```bash
# List CRD versions
kubectl get crd zencleanerpolicies.cleaner.zen-mesh.io -o yaml | grep -A 5 "versions:"
...
Disaster recovery for zen-cleaner requires:
