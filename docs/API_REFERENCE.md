...
Complete API reference for the ZenCleanerPolicy CRD.

## ZenCleanerPolicy

`ZenCleanerPolicy` is a namespaced resource that defines a cleanup policy for Kubernetes resources.
...
- **Plural**: `zencleanerpolicies`
- **Short Names**: `zcp`, `zencleanerpolicy`
...
Defines which resources the cleanup policy applies to.
...
Defines cleanup execution behavior.
...
kubectl get crd zencleanerpolicies.cleaner.zen-mesh.io -o jsonpath='{.spec.versions[0].schema.openAPIV3Schema}' | jq .

# Or view the full CRD
kubectl get crd zencleanerpolicies.cleaner.zen-mesh.io -o yaml
```

The OpenAPI schema is also available in `deploy/crds/cleaner.zen-mesh.io_zencleanerpolicies.yaml`.
...
1. Verify policy is Active:
   ```bash
   kubectl get zencleanerpolicies -o wide
   kubectl describe zencleanerpolicies <policy-name>
   ```

2. Check policy status:
   ```bash
   kubectl get zencleanerpolicies <policy-name> -o jsonpath='{.status}'
   ```
...
1. View policy status:
   ```bash
   kubectl get zencleanerpolicies <policy-name> -o yaml
   ```

2. Check status conditions:
   ```bash
   kubectl get zencleanerpolicies <policy-name> -o jsonpath='{.status.conditions}'
   ```
...
2. Check if conditions are preventing deletion:
   ```bash
   kubectl get zencleanerpolicies <policy-name> -o jsonpath='{.spec.conditions}'
   ```
