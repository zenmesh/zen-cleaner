...
build:
	@echo "Building zen-cleaner..."
	go build -ldflags="-s -w" -trimpath -o bin/zen-cleaner ./cmd/zen-cleaner
	@echo "✅ Build complete: bin/zen-cleaner"
	@ls -lh bin/zen-cleaner | awk '{print "   Binary size: " $$5}'

# Build optimized binary for production
build-release:
	@echo "Building optimized zen-cleaner binary..."
	...
	go build -trimpath \
		...
		-o bin/zen-cleaner ./cmd/zen-cleaner
	@echo "✅ Optimized build complete: bin/zen-cleaner"
	@ls -lh bin/zen-cleaner
...
build-image:
	...
	docker build \
		...
		-t ghcr.io/zenmesh/zen-cleaner:$$VERSION \
		-t ghcr.io/zenmesh/zen-cleaner:latest .
	@echo "✅ Docker image built: ghcr.io/zenmesh/zen-cleaner:$$VERSION"
...
build-image-multiarch:
	...
	docker buildx build \
		...
		-t ghcr.io/zenmesh/zen-cleaner:latest \
		--push .
	@echo "✅ Multi-arch Docker images built: ghcr.io/zenmesh/zen-cleaner:$$VERSION"
...
# Usage: make test-e2e CLUSTER_NAME=zen-cleaner-e2e
test-e2e:
